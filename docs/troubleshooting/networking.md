...
-   In system settings, set the location access permission for Home Assistant to 'Always' on iOS or 'Allow all the time' on Android. This is required because iOS 13 and later and all Android versions will only let apps with such permission have access to the wifi SSID which is used by the app to determine whether to use internal or external URLs.
